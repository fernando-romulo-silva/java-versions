...
class Main01 {
    ...
    // but it also allows child thread to access the thread-local values from the parent thread.
    public static InheritableThreadLocal<String> CONTEXT = new InheritableThreadLocal();
    //
    // So if we create a million threads, we must create a million ThreadLocal instances also. 
    // Such a large number of instances can lead to degraded performance in resource-intensive applications. 
